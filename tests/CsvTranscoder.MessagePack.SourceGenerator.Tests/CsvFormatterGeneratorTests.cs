...
public class CsvFormatterGeneratorTests
{
    ...
    // -----------------------------------------------------------------------
    //  LocalizedMember support
    // -----------------------------------------------------------------------

    [Fact]
    public void GeneratedFormatter_LocalizedMemberKey_UsesLocalizedMemberFormatter()
    {
        // [LocalizedMember, Key(1)] string members must use the localized-member formatter
        // directly instead of the resolver, so that the extra language CSV column is consumed.
        const string source = """
            using MessagePack;
            using Localization;
            using AndanteTribe.Csv;
            namespace MyNs
            {
                ...
                {
                    [Key(0)] public int Id { get; set; }
                    [LocalizedMember, Key(1)] public string Name { get; set; } = "";
                }
                [GeneratedCsvFormatterResolver]
                public partial class MyResolver { }
            }
        ...
        var formatterSrc = GetGeneratedSource(compilation, "EntityCsvFormatter");
        Assert.NotNull(formatterSrc);

        // The [LocalizedMember] property must use the dedicated localized formatter…
        Assert.Contains("LocalizedMemberJapaneseCsvFormatter", formatterSrc);
        // …and must NOT use the generic resolver path for the Name member.
        Assert.DoesNotContain("GetFormatterWithVerify<string>", formatterSrc);
    }

    [Fact]
    public void GeneratedFormatter_NonLocalizedStringKey_UsesResolverPath()
    {
        // A plain string Key without [LocalizedMember] should still use the resolver.
        const string source = """
            using MessagePack;
            using AndanteTribe.Csv;
            namespace MyNs;
        ...
        // Without [LocalizedMember], the resolver path is used for string.
        Assert.Contains("GetFormatterWithVerify<string>", formatterSrc);
        Assert.DoesNotContain("LocalizedMemberJapaneseCsvFormatter", formatterSrc);
    }
}